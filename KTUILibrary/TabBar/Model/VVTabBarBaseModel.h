...
FOUNDATION_EXPORT NSString * const kVVTabBarSchemeKey;

#define VVTABBARFILE(host) \
[NSURL URLWithString:[NSString stringWithFormat:@"%@://%@", kVVTabBarSchemeKey, host]]

typedef NS_ENUM(NSUInteger, VVTabBarImageType) {
    /// lottie动画
    VVTabBarImageTypeLot,
    /// image
    VVTabBarImageTypeImage,
    /// gif动画
    VVTabBarImageTypeGif,
};

/// VVTabBar按钮配置项基类
@interface VVTabBarBaseModel : NSObject

/// tabbar图片加载方式
@property (nonatomic, assign) VVTabBarImageType imageType;
