...
@property (nonatomic, strong) VVTabBarBaseModel *model;

@end

@implementation VVTabBarBaseButton

+ (id)configTabBarButtonWithModel:(VVTabBarBaseModel *)model
{
    __kindof VVTabBarBaseButton *button = [[model.className alloc] initModel:model];
    return button;
}

- (instancetype)initModel:(VVTabBarBaseModel *)model
{
    self = [super init];
    if (self) {
        self.model = model;
...
- (void)updateWithModel:(VVTabBarBaseModel *)model
{
    if (![model isKindOfClass:[VVTabBarBaseModel class]]) {
#if DEBUG
        NSAssert(NO, @"模型不对");
#endif
        return;
...
/// 设置背景图
- (void)setUpBGImageView:(VVTabBarBaseModel *)model
{
    NSURL *url;
    if (model.selected) {
    ...
    if ([url.scheme isEqualToString:kVVTabBarSchemeKey]) {
...
/// 按imageType添加需要的view
- (void)configImageView:(VVTabBarBaseModel *)model
{
...
//        case VVTabBarImageTypeLot:
//        {
//            [self addSubview:self.lotImageView];
//        }
//            break;

        case VVTabBarImageTypeImage:
        {
            [self addSubview:self.normalImageView];
        }
            break;
            
//        case VVTabBarImageTypeGif:
//        {
...
- (void)setUpImageView:(VVTabBarBaseModel *)model
{
...
/// 设置lottie的tabbar
- (void)setUpLotType:(VVTabBarBaseModel *)model
{
...
//        return;
//    }
//    if ([model.lottieURL.scheme isEqualToString:kVVTabBarSchemeKey]) {
...
- (void)lottiePlayAnimationIfNeeded:(VVTabBarBaseModel *)model
{
#warning TODO 0303
//    if (model.selected) {
...
/// 设置image的tabbar
- (void)setUpImageType:(VVTabBarBaseModel *)model
{
...
/// 设置gif的tabbar
- (void)setUpGifType:(VVTabBarBaseModel *)model
{
#warning TODO 0303
...
- (void)remakeConstraints:(UIView *)view model:(VVTabBarBaseModel *)model
{
...
/// 设置tabbar的文字
- (void)setUpTitle:(VVTabBarBaseModel *)model
{
    if ((model.selected &&
